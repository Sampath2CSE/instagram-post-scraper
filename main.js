...
const extractInstagramData = async (page, url) => {
    try {
        ...
        if (isProfileUrl) {
            // Handle profile URL - navigate and extract profile data
            await page.goto(url, { 
            ...
            const profileResult = await page.evaluate(() => {
                ...
                // Try to find Instagram's JSON data in script tags
                ...
                for (const script of scripts) {
                    try {
                        ...
                        const patterns = [
                            ...
                            /"ProfilePage"\]\],\[.*?"user":\s*({.+?}),"logging_page_id"/,
                            // Pattern 3: Direct user data
                            /"graphql":\s*{"user":\s*({.+?})},"hostname"/
                        ];
                            ...
                            if (match) {
                                try {
                                    ...
                                    if (data.entry_data?.ProfilePage?.[0]?.graphql?.user) {
                                        return { profileData: data.entry_data.ProfilePage[0].graphql.user };
                                    }
                ...
                console.log('Script method failed, trying DOM extraction for post URLs...');
                const links = [];
                ...
                return { postUrls: [...new Set(links)] };
            });
            
            return profileResult;
        } else {
            // Handle individual post/reel URL
...
const getProfilePosts = async (page, username, maxPosts) => {
    try {
        ...
        // Use the postUrls from DOM fallback if available
        if (result && result.postUrls && result.postUrls.length > 0) {
            console.log(`Fallback method found ${result.postUrls.length} URLs`);
            return result.postUrls.slice(0, maxPosts);
        }
